...
async function initializeApp(): Promise<firebase.app.App> {
  const serviceAccount = await fs.promises.readFile(
...
async function main(): Promise<void> {
  ...
  const itemsQuery = firestore
    ...
    .doc(projectId)
    .collection('items')
    .orderBy('last30Days.minMaxRate', 'desc')
    .endAt(0.5);
  const itemsSnapshot = await itemsQuery.get();
  console.log(`${itemsSnapshot.size} items`);
  const result = await itemsSnapshot.docs.reduce(
    async (
      promise,
      itemSnapshot,
      ...
      const pricesQuery = itemSnapshot.ref
        .collection('prices')
  ...
  await fs.promises.writeFile(
    `${__dirname}/data/items.json`,
    JSON.stringify({ items: result }, null, 2),
  );
}
main().then(
  () => console.log('OK'),
