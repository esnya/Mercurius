...
def preprocess(prices):
    ...
    resampled = quantized.resample('3H').mean()
    ...
    normalized = interpolated.transform({
      ...
      'price': lambda price: price / 1000000,
      'lottery': lambda x: x,
    })
    ...
    benefits.loc[raiseAndFall['index'] < 0, :] = 0
    benefitIndices = benefits.applymap(lambda x: 1 if x >= 1 else 0)

    return normalized, benefitIndices

def toTrainSet(normalized, benefitIndices):
    trainSet = [(normalized[left:left + pd.to_timedelta('7D') - pd.to_timedelta('3H')], benefitIndices[left + pd.to_timedelta('7D'):left + pd.to_timedelta('7D') + pd.to_timedelta('2D') - pd.to_timedelta('3H')]) for left in normalized.index]
